...
document.querySelectorAll("[data-feature-tab]").forEach((tab)=>{
  tab.addEventListener("click",()=>{
    const index=tab.dataset.featureTab;
    document.querySelectorAll("[data-feature-tab]").forEach((item)=>item.classList.toggle("active",item===tab));
    document.querySelectorAll("[data-feature-panel]").forEach((panel)=>panel.classList.toggle("active",panel.dataset.featurePanel===index));
  });
});
const suiteWord=document.querySelector("[data-suite-word]");
if(suiteWord){
  const suiteWords=["Liquidaci&oacute;n","Portal RR.HH.","Control horario","Leia AI"];
  let suiteWordIndex=0;
  setInterval(()=>{
    suiteWordIndex=(suiteWordIndex+1)%suiteWords.length;
    suiteWord.classList.add("leaving");setTimeout(()=>{suiteWord.innerHTML=suiteWords[suiteWordIndex];suiteWord.classList.remove("leaving");},260);
  },1800);
}

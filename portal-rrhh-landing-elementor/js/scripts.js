...
if(suiteWord){
  const suiteWords=["Portal RR.HH.","Liquidaci&oacute;n","Control horario","Leia AI"];
  let suiteWordIndex=0;
  setInterval(()=>{
    suiteWordIndex=(suiteWordIndex+1)%suiteWords.length;
    suiteWord.classList.add("leaving");
    setTimeout(()=>{suiteWord.innerHTML=suiteWords[suiteWordIndex];suiteWord.classList.remove("leaving");},260);
  },1800);
}
